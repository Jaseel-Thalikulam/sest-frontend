import {
  Box,
  Avatar,
  Typography,
  Chip,
  Button,
} from "@mui/material";
import { useEffect, useState } from "react";
import { useParams } from "react-router-dom";
import "./tutorDetailPage.scss";
import ChatUI from "../../components/ChatUI-Modal/ChatUI";


import { useMediaQuery, useTheme } from "@mui/material";
import axiosInstanceStudent from "../../interceptor/axiosInstance.Student";
import IUserSlice from "../../../interface/Iredux/IuserSlice";
import IFetchTutorResponse from "../../../interface/TutorDetailPage/fetchTutor.Interface";
import NavBar from "../../../common/Components/NavBar/NavBar";
import ChatUIModal from "../../modal/ChatUI";

function TutorDetailPage() {
  const { tutorId } = useParams();
  const [profileData, setProfileData] = useState<IUserSlice>();
  const theme = useTheme();
  const isSmallScreen: boolean = useMediaQuery(theme.breakpoints.down("sm"));


  function properCase(inputString: string) {
    return inputString.replace(/\b\w/g, function (match) {
      return match.toUpperCase();
    });
  }

  const [chatUI, setChatUI] = useState(false);

  function handleChatUI() {
    setChatUI(!chatUI);
  }

  useEffect(() => {
    const fetchTutor = async () => {
      try {
        const response: { data: IFetchTutorResponse } =
          await axiosInstanceStudent.post("/tutordata", {
            tutorId,
          });

        const tutordata = response.data.Tutorsdata;

        setProfileData(tutordata);
      } catch (error) {
        console.error("Error fetching tutor data:", error);
      }
    };

    fetchTutor().catch((error) => {
      console.error("Error in fetchTutor:", error);
    });
  }, [tutorId]);

  return (
    <>
      <NavBar isSmallScreen={isSmallScreen} />
      <div className="profile-container">
        {profileData && (
          <Box className="profile-details">
            {profileData.avatarUrl !== null ? (
              <Avatar
                alt={profileData.name}
                src={profileData.avatarUrl}
                className="avatar"
              />
            ) : null}
            <div className="info-container">
              <Typography variant="h4" className="name">
                {properCase(profileData.name)}
              </Typography>
              {profileData.tags !== null ? (
                <Box className="tag-chips">
                  {profileData.tags.map((tag, index) => (
                    <Chip key={index} label={tag.Name} variant="outlined" />
                  ))}
                </Box>
              ) : null}

              <div className="button-container">
                <Button
                  variant="contained"
                  color="primary"
                  className="follow-button"
                >
                  Follow
                </Button>
                <Button
                  variant="contained"
                  color="success"
                  className="follow-button"
                  onClick={() => void handleChatUI()}
                >
                  Chat
                </Button>
              </div>

              <Typography variant="body1" className="about">
                {profileData.about}
              </Typography>
              <Typography variant="h6" className="courses">
                Courses Released
              </Typography>
            </div>
          </Box>
        )}
      </div>

      <ChatUIModal
        CloseModal={() => void handleChatUI()}
        isOpen={chatUI}
      >
        <ChatUI
          recipientName={profileData ? properCase(profileData.name) : ""}
          recipientAvatarUrl={profileData ? profileData.avatarUrl : ""}
        />
      </ChatUIModal>
    </>
  );
}

export default TutorDetailPage;
